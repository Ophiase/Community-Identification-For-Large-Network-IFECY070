...
from .graph_generation import erdos_graph_m
...
def degree_distribution(graph: nx.Graph, num_destinations: int) -> Dict[int, int]:
    """
    ...
    destinations = set(random.sample(nodes, min(num_destinations, len(nodes))))
...
def plot_degree_distribution(distribution: Dict[int, int]) -> None:
...
def compute(graph: nx.Graph, num_destinations: int):
    distribution = degree_distribution(graph, num_destinations)
